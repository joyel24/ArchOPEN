...
uint32_t HW_cpld::read(uint32_t addr,int size)
{
    ...
    switch(addr)
    {
        ...
        case CPLD_START+CPLD_PORT_OFFSET+0xc:
            ret_val = 0;
            DEBUG_HW(CPLD_HW_DEBUG,"CPLD6 - read, size %x: %x\n",size,ret_val);
            break;
// Buttons
        case CPLD_START+0x600008:
            BTN_FCT(btn->btn_var[BTN_UP]     ,0x1)
            BTN_FCT(btn->btn_var[BTN_DOWN]   ,0x2)
            BTN_FCT(btn->btn_var[BTN_LEFT]   ,0x4)
            BTN_FCT(btn->btn_var[BTN_RIGHT]  ,0x8)
        ...
        case CPLD_START+0x60000c:
            BTN_FCT(btn->btn_var[BTN_SQUARE] ,0x1)
            BTN_FCT(btn->btn_var[BTN_CROSS]  ,0x2)
            DEBUG_HW(CPLDBTN_HW_DEBUG,"CPLD - read buttons (@0x%08x), size %x => send %x\n",addr,size,ret_val);
            break;
...
void HW_cpld::write(uint32_t addr,uint32_t val,int size)
{
    ...
    {
        case CPLD_START+CPLD_PORT_OFFSET+0x0:
            DEBUG_HW(CPLD_HW_DEBUG,"CPLD0 - write, size %x: %x\n",size,val);
            cpld_ata_mode = val;
            cpld_val[0] = val;
            break;
        case CPLD_START+CPLD_PORT_OFFSET+0x2:
        ...
        case CPLD_START+CPLD_PORT_OFFSET+0x4:
            ...
            break;
        case CPLD_START+CPLD_PORT_OFFSET+0x6:
            DEBUG_HW(CPLD_HW_DEBUG,"CPLD3 - write, size %x: %x \n",size,val);
            cpld_val[3] = val;
            break;
        case CPLD_START+CPLD_PORT_OFFSET+0x8:
        case CPLD_START+CPLD_PORT_OFFSET+0xa:
        case CPLD_START+CPLD_PORT_OFFSET+0xc:
            DEBUG_HW(CPLD_HW_DEBUG,"CPLD4/5/6 - write, size %x: %x NOT supported\n",size,val);
            break;
        case CPLD_START+0x600008:
        case CPLD_START+0x60000a:
        case CPLD_START+0x60000c:
...
{
    printf("CPLD: write state:\n-0:%04x\n-1:%04x\n-2:%04x\n-3:%04x\n",cpld_val[0],cpld_val[1],
           cpld_val[2],cpld_val[3]);
}
