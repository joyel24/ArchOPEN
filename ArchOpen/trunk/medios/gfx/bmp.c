...
unsigned int numberColor(int bitcount)
{
    unsigned int res;
    switch(bitcount)
    {
...
void read1BitRow(unsigned int * buffer,char * data,int width)
{
    int i,j,k;;
    bmpColor * tmp,*tmp2;
    int hasMore=width%8;
...
void read4BitRow(unsigned int * buffer,char * data,int width)
{
    int i;
    bmpColor * tmp,*tmp2;
...
void read8BitRow(unsigned int * buffer,char * data,int width)
{
    int i;
    bmpColor * tmp,*tmp2;
    int hasMore=width%2;
...
void read16BitRow(unsigned int * buffer,char * data,int width)
{
}

void read24BitRow(unsigned int * buffer,char * data,int width)
{
    int i,k;
...
void read32BitRow(unsigned int * buffer,char * data,int width)
{
...
MED_RET_T gfx_loadBmp(char * filename)
{
    ...
    if(infoHeader.bitcount<16)
    {
        ...
        /*creating palette*/
        bmpPalette=(bmpColor*)malloc(numberColor(infoHeader.bitcount)*sizeof(bmpColor));
        if(!bmpPalette)
        {
            printk("[BMP ERROR] error doing malloc for palette (size=%d)\n",
                   numberColor(infoHeader.bitcount)*sizeof(bmpColor));
            close(fd);
            free(palette_buf);
            return -MED_ERROR;
        }

        printk("Reading palette: %d colors / %d\n",paletteSize,numberColor(infoHeader.bitcount));
        ...
        for(i=j;i<numberColor(infoHeader.bitcount);i++)
    ...
    else
    {
        ...
        for(i=infoHeader.height-1;i>=0;i--)
        {
            ...
            switch(bitDepth)
            {
                case 1 : read1BitRow(buff_offset,tmpRow,width); break;
                case 4 : read4BitRow(buff_offset,tmpRow,width); break;
                case 8 : read8BitRow(buff_offset,tmpRow,width); break;
                case 24: read24BitRow(buff_offset,tmpRow,width); break;
                case 32: read32BitRow(buff_offset,tmpRow,width); break;
            }
