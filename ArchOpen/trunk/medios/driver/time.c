...
int (*getOffset)(void);
MED_RET_T (*getTime)(struct med_tm * valTime);
    
int calcMonYear(int type,int year,int org,int * offset)
{
    int sig=1;
    if(*offset==0)
        return org;
    ...
    while(*offset>0)
    {
        *offset=*offset-(type==0?nbSec_year(org):month_Sectab[isLeap(year)][org]);
        org=org+sig;
    }
    ...
    {
        org=org-sig;
        *offset+=(type==0?nbSec_year(org):month_Sectab[isLeap(year)][org]);
    }
...
{    
    while(1)
    {
        if(*val<lower)
        {
//             printk("below: %d %d-%d",*val,lower,upper);
            *val=upper-(lower-*val-1);
            *upperVal=*upperVal-1;      
        }
        else if(*val>upper)
        {
//             printk("upper %d %d",*val,upper);
...
{    
    ...
    while(1)
    {
        ...
        {
            //printk("below: %d %d",val_time->tm_mday,1);
        ...
        {
           // printk("upper %d %d",val_time->tm_mday,month_tab[isLeap(val_time->tm_year)][val_time->tm_mon]);
...
void time_calcOffset(struct med_tm * val_time,int offset_val)
{
    ...
    /* apply offset */
    //printk("offset= %x\n",offset);
    val_time->tm_year=calcMonYear(0,0,val_time->tm_year,&offset);
    //printk("y=%d offset= %x\n",val_time->tm_year,offset);
    val_time->tm_mon=calcMonYear(1,val_time->tm_year,val_time->tm_mon,&offset);
    //printk("m=%d offset= %x\n",val_time->tm_mon,offset);
