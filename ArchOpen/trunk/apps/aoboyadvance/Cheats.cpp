...
void cheatsSaveCheatList(char *file)
{
  ...
  if(f<0)
    return;
  int version = 1;
  write(f, &version, sizeof(version));
  int type = 0;
  write(f, &type, sizeof(type));
  write(f, &cheatsNumber, sizeof(cheatsNumber));
  write(f, cheatsList, sizeof(cheatsList));
  close(f);
}
...
bool cheatsLoadCheatList(char *file)
{
  ...
  if(read(f, &version, sizeof(version)) != sizeof(version)) {
    close(f);
    return false;
  }
  ...
  int type = 0;
  if(read(f, &type, sizeof(type)) != sizeof(type)) {
    close(f);
    return false;
  }
  ...
  if(read(f, &count, sizeof(count)) != sizeof(count)) {
    close(f);
    return false;
  }

  if(read(f, cheatsList, sizeof(cheatsList)) != sizeof(cheatsList)) {
    close(f);
    return false;
  }
