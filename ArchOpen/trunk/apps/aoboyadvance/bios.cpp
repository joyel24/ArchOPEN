...
void BIOS_ArcTan()
{
  ...
  s32 a = -((s32)(registers[0] * registers[0])) >> 14; 
...
void BIOS_ArcTan2()
{
  ...
  if (y == 0) {
    registers[0] = 0x8000 & x;
    registers[3] = 0x170;
  } else {
    if (x == 0) {
      registers[0] = (0x8000 & y) + 0x4000;
      registers[3] = 0x170;
    } else {
      if (abs(x) > abs(y)) {
        registers[1] = x;
        ...
        BIOS_ArcTan();
        if (x < 0)
          registers[0] = 0x8000 + registers[0];
        else
          registers[0] = ((y & 0x8000) << 1 ) + registers[0];
        registers[3] = 0x170;
      } else {
        registers[0] = x << 14;
        BIOS_Div();
        BIOS_ArcTan();
        registers[0] = (0x4000 + (y & 0x8000)) - registers[0];
        registers[3] = 0x170;
      }
    }
  }
...
void BIOS_BitUnPack()
{
  ...
  u32 source = registers[0];
  u32 dest = registers[1];
  u32 header = registers[2];
...
void BIOS_Div()
{
  ...
  int number = registers[0];
  int denom = registers[1];

  if(denom != 0) {
    registers[0] = number / denom;
    registers[1] = number % denom;
    s32 temp = (s32)registers[0];
    registers[3] = temp < 0 ? (u32)-temp : (u32)temp;
  }
#ifdef DEV_VERSION
  if(systemVerbose & VERBOSE_SWI) {
...
void BIOS_RegisterRamReset()
{
  ...
  BIOS_RegisterRamReset(registers[0]);
}
...
/*
  ...
  registers[R14_IRQ] = 0x00000000;
  registers[SPSR_IRQ] = 0x00000000;
  registers[R13_SVC] = 0x03007FE0;
  registers[R14_SVC] = 0x00000000;
  registers[SPSR_SVC] = 0x00000000;
...
#endif
  registers[0] = (u32)sqrt((double)registers[0]);
#ifdef DEV_VERSION
  if(systemVerbose & VERBOSE_SWI) {
    log("Sqrt: return=%08x\n",
        registers[0]);
...
#endif
  int freq = CPUReadMemory(registers[0]+4);
  double tmp;
  tmp = ((double)(180 - registers[1])) - ((double)registers[2] / 256.f);
  tmp = pow((double)2.f, tmp / 12.f);
  registers[0] = (int)((double)freq / tmp);
...
void BIOS_SndDriverJmpTableCopy()
{
  ...
  for(int i = 0; i < 0x24; i++) {
    CPUWriteMemory(registers[0], 0x9c);
    registers[0] += 4;
  }
}
