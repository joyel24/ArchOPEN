...
bool utilWriteBMPFile(char *fileName, int w, int h, u8 *pix)
{
  ...
  write(fp, &bmpheader, sizeof(bmpheader));
...
gzFile utilGzOpen(char *file, int mode)
{
  return -1;
}

gzFile utilMemGzOpen(char *memory, int available, int mode)
{
  return -1;
}
