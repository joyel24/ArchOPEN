...
// Area.c
typedef size_t (arearw)(int file, void *p, size_t _size);
// Save or load the state from PmovFile:
int PmovState(int PmovAction, int PmovFile); // &1=for reading &2=for writing &4=volatile &8=non-volatile
extern arearw *areaRead;  // read and write function pointers for
extern arearw *areaWrite; // gzip save state ability

// Cart.c
int PicoCartLoad(int f,unsigned char **prom,unsigned int *psize);
...
// Draw.c
void DrawSetColorFormat(int which); // 0=BGR444, 1=RGB555, 2=8bit(HighPal pal)
extern void *DrawLineDest;
extern int (*PicoScan)(unsigned int num, void *data);
// internals
extern unsigned int HighPal[0x100];
...
extern unsigned int *PicoCramHigh; // for 32 bits YUV rendering...
