...
#ifdef __BROKEN_FWRITE
// Symbian OS 9.1: fwrite needs capabilities ???
// FIXME
static unsigned int dummy_fwrite(void *a1, unsigned int a2, unsigned int a3, void *a4) { return 0; }
#undef  fwrite
...
arearw *areaRead  = (arearw *) read;  // read and write function pointers for
arearw *areaWrite = (arearw *) write; // gzip save state ability
...
{
  int ret = 0;
  if ((PmovAction&3)==1) ret = areaWrite(PmovFile,data,len);
  if ((PmovAction&3)==2) ret = areaRead (PmovFile,data,len);
  return (ret != len);
}
...
#ifdef SOUND_USE_DSP
	if(PicoOpt&3)
      ScanVar(dspCom->sn76496_regs,8*2,"SN76496state", PmovFile, PmovAction); // regs and other stuff
	if(PicoOpt&1) {
      ScanVar(&dspCom->ym2612_regs[0], 0x400, "YM2612state", PmovFile, PmovAction); // regs
      ScanVar(&dspCom->addr_A1, 2, "YM2612AddrA1", PmovFile, PmovAction); // addr line
...
int PmovState(int PmovAction, int PmovFile)
{
  int minimum=0;
  ...
  *(unsigned int *)(head+0xc)=minimum;
  ...
  // Scan header:
  if (PmovAction&1) areaWrite(PmovFile,head,sizeof(head));
  if (PmovAction&2) areaRead (PmovFile,head,sizeof(head));
