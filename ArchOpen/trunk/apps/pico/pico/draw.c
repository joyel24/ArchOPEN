...
void (*FinalizeLine)(int sh) = FinalizeLineRGB555;
...
__IRAM_DATA static unsigned int HighCol[32+320+8]; // Gap for 32 column, and messy border on right
unsigned int HighPal[0x100];

static int Scanline=0; // Scanline

int rendstatus;

static int TileNorm(unsigned int *pd,int addr,unsigned int *pal)
{
  unsigned int pack=0; unsigned int t=0;
  ...
  if (pack)
  {
    t=pack&0x0000f000; if (t) { t=pal[t>>12]; pd[0]=(unsigned int)t; }
    t=pack&0x00000f00; if (t) { t=pal[t>> 8]; pd[1]=(unsigned int)t; }
    t=pack&0x000000f0; if (t) { t=pal[t>> 4]; pd[2]=(unsigned int)t; }
    t=pack&0x0000000f; if (t) { t=pal[t    ]; pd[3]=(unsigned int)t; }
    t=pack&0xf0000000; if (t) { t=pal[t>>28]; pd[4]=(unsigned int)t; }
    t=pack&0x0f000000; if (t) { t=pal[t>>24]; pd[5]=(unsigned int)t; }
    t=pack&0x00f00000; if (t) { t=pal[t>>20]; pd[6]=(unsigned int)t; }
    t=pack&0x000f0000; if (t) { t=pal[t>>16]; pd[7]=(unsigned int)t; }
    return 0;
  }
...
static int TileFlip(unsigned int *pd,int addr,unsigned int *pal)
{
  unsigned int pack=0; unsigned int t=0;
  ...
  if (pack)
  {
    t=pack&0x000f0000; if (t) { t=pal[t>>16]; pd[0]=(unsigned int)t; }
    t=pack&0x00f00000; if (t) { t=pal[t>>20]; pd[1]=(unsigned int)t; }
    t=pack&0x0f000000; if (t) { t=pal[t>>24]; pd[2]=(unsigned int)t; }
    t=pack&0xf0000000; if (t) { t=pal[t>>28]; pd[3]=(unsigned int)t; }
    t=pack&0x0000000f; if (t) { t=pal[t    ]; pd[4]=(unsigned int)t; }
    t=pack&0x000000f0; if (t) { t=pal[t>> 4]; pd[5]=(unsigned int)t; }
    t=pack&0x00000f00; if (t) { t=pal[t>> 8]; pd[6]=(unsigned int)t; }
    t=pack&0x0000f000; if (t) { t=pal[t>>12]; pd[7]=(unsigned int)t; }
    return 0;
  }
  return 1; // Tile blank
}
...
__IRAM_CODE static int DrawStrip(struct TileStrip *ts)
{
  ...
  int code=0,addr=0,zero=0, oldcode=-1;
  unsigned int *pal=NULL;
  unsigned int pack=0; unsigned int t=0; unsigned int *pd;
...
#ifdef PROFILING
  //profile_stop("DrawStrip");
#endif
  return 0;
}

__IRAM_CODE static int DrawLayer(int plane,int high)
{
  struct PicoVideo *pvid=&Pico.video;
  static char shift[4]={5,6,6,7}; // 32,64 or 128 sized tilemaps
  struct TileStrip ts;
#ifdef PROFILING
  //profile_start("DrawLayer");
#endif
  // Work out the TileStrip to draw
...
__IRAM_CODE static int DrawSprite(int sy,unsigned short *sprite,int high)
{
  ...
  int i=0;
#ifdef PROFILING
  //profile_start("DrawSprite");
#endif

  code=sprite[2];
  if ((code>>15)!=high) { WrongPri=1;
#ifdef PROFILING
  //profile_stop("DrawSprite");
#endif
  ...
  width=(height>>2)&3; height&=3;
  width++; height++; // Width and height in tiles
  if (Scanline>=sy+(height<<3))
  {
#ifdef PROFILING
   //                             profile_stop("DrawSprite");
#endif
                                return 0; // Not on this line after all
  }

  row=Scanline-sy; // Row of the sprite we are on
  pal=HighPal+((code>>9)&0x30); // Get palette pointer
  if (code&0x1000) row=(height<<3)-1-row; // Flip Y
  ...
  i=width;
  unsigned int *HighCol2=HighCol+24;
  while (i--)
  {
    if (sx<=0 || sx>=328) continue; // Offscreen

    tile&=0x7fff; // Clip tile address
    if (code&0x0800) TileFlip(HighCol2+sx,tile,pal);
    else             TileNorm(HighCol2+sx,tile,pal);
...
__IRAM_CODE static int DrawAllSprites(int high)
{
  struct PicoVideo *pvid=&Pico.video;
  int table=0;
  int i=0,link=0;
...
/*int UpdatePalette()
{
  ...
  // Update palette:
  for (c=0;c<64;c++) HighPal[c]=(unsigned int)PicoCram(Pico.cram[c]);
  Pico.m.dirtyPal=0;
...
static int Overlay()
{
  ...
  for (x=0;x<4;x++) HighCol[32+x]=(unsigned int)col;
*/
  return 0;
}
...
int PicoLine(int scan)
{
 ...
 //profile_start("PicoLine");
  ...
  else
  {
    ...
    Skip=PicoScan(Scanline,HighCol+32);
  }

#ifdef PROFILING
  //profile_stop("PicoLine");
#endif
  return 0;
}
